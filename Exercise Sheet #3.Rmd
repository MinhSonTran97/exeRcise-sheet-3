...
```
```{r pixar-metacritic, fig.cap= "How well do the sequels perform compared to the first installments"}
ggplot(
  data = pixar_series,
  aes(
    x = series,
...
We can see in Figure \@ref(fig:pixar-metacritic) that the first installments in each series all have the
highest Metacritic score. After that there, is a clear downward trend in
most series, meaning that the sequels usually couldn't perform as well
as the prequels.
...
Research question: How has the global adult literacy rate changed
between 1970 and 2015? 

The period from 1970 to 2015 includes major global education initiatives, such as UNESCO’s Education for All, making it an interesting and relevant timeframe to assess how these efforts have impacted adult literacy worldwide.
...
The global mean adult literacy rate rose from `r round(literacy_change$avg_world_literacy[1],2)`% in `r round(literacy_change$Year[1])` to `r round(literacy_change$avg_world_literacy[2],2)`% in `r round(literacy_change$Year[2])`, an increase of about `r round(literacy_change$change[2],2)`% over 45 years. While the increase may seem modest, it represents millions of additional literate adults globally, which is a significant improvement, especially in regions starting with lower literacy.
